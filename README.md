...
``` bash
$ fadmin init webpack my-project
```

The above command pulls the template from [fadminjs-templates/webpack](https://github.com/fadminjs-templates/webpack), prompts for some information, and generates the project at `./my-project/`.

### fadmin build

Use fadmin-cli as a zero-configuration development tool for your fadmin apps and component, check out the [docs](/docs/build.md).

### Official Templates

The purpose of official fadmin project templates are to provide opinionated, battery-included development tooling setups so that users can get started with actual app code as fast as possible. However, these templates are un-opinionated in terms of how you structure your app code and what libraries you use in addition to fadmin.js.

All official project templates are repos in the [fadminjs-templates organization](https://github.com/fadminjs-templates). When a new template is added to the organization, you will be able to run `fadmin init <template-name> <project-name>` to use that template. You can also run `fadmin list` to see all available official templates.

Current available templates include:

- [webpack](https://github.com/fadminjs-templates/webpack) - A full-featured Webpack + fadmin-loader setup with hot reload, linting, testing & css extraction.

- [webpack-simple](https://github.com/fadminjs-templates/webpack-simple) - A simple Webpack + fadmin-loader setup for quick prototyping.

- [browserify](https://github.com/fadminjs-templates/browserify) - A full-featured Browserify + fadminify setup with hot-reload, linting & unit testing.

- [browserify-simple](https://github.com/fadminjs-templates/browserify-simple) - A simple Browserify + fadminify setup for quick prototyping.

- [pwa](https://github.com/fadminjs-templates/pwa) - PWA template for fadmin-cli based on the webpack template

- [simple](https://github.com/fadminjs-templates/simple) - The simplest possible fadmin setup in a single HTML file

### Custom Templates

It's unlikely to make everyone happy with the official templates. You can simply fork an official template and then use it via `fadmin-cli` with:

``` bash
fadmin init username/repo my-project
```
...
``` bash
fadmin init ~/fs/path/to-custom-template my-project
```
...
``` json
{
  "skipInterpolation": "src/**/*.fadmin"
}
```

#### Metalsmith

`fadmin-cli` uses [metalsmith](https://github.com/segmentio/metalsmith) to generate the project.

You may customize the metalsmith builder created by fadmin-cli to register custom plugins.
...
- `helpers`: some helpers you can use to log results.
  ...
  - `logger`: [the built-in fadmin-cli logger](/lib/logger.js)
...
`fadmin-cli` uses the tool [`download-git-repo`](https://github.com/flipxfx/download-git-repo) to download the official templates used. The `download-git-repo` tool allows you to indicate a specific branch for a given repository by providing the desired branch name after a pound sign (`#`).

The format needed for a specific official template is:

```
fadmin init '<template-name>#<branch-name>' <project-name>
```

Example:

Installing the [`1.0` branch](https://github.com/fadminjs-templates/webpack-simple/tree/1.0) of the webpack-simple fadmin template:

```
fadmin init 'webpack-simple#1.0' mynewproject
```
